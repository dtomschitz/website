export type User = {
  firstName: string;
  lastName: string;
  /** Preferred public-facing name */
  displayName: string;
  /** Handle/username used in links or mentions */
  username: string;
  gender: 'male' | 'female' | 'non-binary';
  /** e.g. "he/him", "she/her", "they/them" */
  pronouns: string;
  bio: string;
  /** Short phrases rotated in UI (homepage flip effect) */
  flipSentences: string[];
  /** General location for display */
  address: string;
  /** base64 encoded email (avoids plaintext in served HTML) */
  emailB64: string;
  /** Personal/homepage URL */
  website: string;
  /** Primary/current role shown on profile */
  jobTitle: string;
  /** Work history entries shown in the Overview block */
  jobs: {
    title: string;
    company: string;
    website: string;
    experienceId?: string;
  }[];
  /** Rich about section; supports Markdown */
  about: string;
  /** Public URL or /public path to avatar image */
  avatar: string;
  /** Open Graph image URL for social sharing */
  ogImage: string;
  /** Time zone in IANA format (e.g., "Europe/Berlin") */
  timeZone: string;
  /** SEO keywords list for metadata */
  keywords: string[];
  /** Profile/site start date in YYYY-MM-DD */
  dateCreated: string;
};
