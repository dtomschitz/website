...
function PanelHeader({className, ...props}: React.ComponentProps<'header'>) {
  return (
    <header
      ...
      className={cn(
        'screen-line-bottom px-4 py-2 has-data-[slot=panel-description]:*:data-[slot=panel-title]:screen-line-bottom',
        className,
      )}
      {...props}
    />
...
function PanelTitle({className, asChild = false, ...props}: React.ComponentProps<'h2'> & {asChild?: boolean}) {
